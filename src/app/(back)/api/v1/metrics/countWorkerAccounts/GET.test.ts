...
const getMetricsAccount = async ({
  ...
  const response = await fetch(
    `http://localhost:3000/api/v1/metrics/countWorkerAccounts${searchParams}`,
    {
      headers: {
        cookie,
      },
...
describe("GET on `/api/v1/metrics/countManagerAccounts`", () => {
  ...
  describe("root user", () => {
    it("should be return worker accounts metrics from last 2 months grouped by day if any search params is provided", async () => {
      const { response, data } = await getMetricsAccount({
        cookie: rootCookie,
      });

      expect(response.status).toStrictEqual(200);
      expect(data.totalWorkers).toStrictEqual(22);
      expect(data.period).toStrictEqual("day");
      expect(Object.entries(data.newWorkersPerPeriod).length).toStrictEqual(21);
    });
    ...
    it("should be return author accounts metrics from last 1 months grouped by day if inicialDate and finalDate params is provided", async () => {
      ...
      expect(data.totalWorkers).toStrictEqual(22);
      expect(data.period).toStrictEqual("day");
      Object.entries(data.newWorkersPerPeriod).forEach(([key, value]) => {
    ...
    it("should be return author accounts metrics grouped by day", async () => {
      ...
      Object.entries(data.newWorkersPerPeriod);
    });
  });
});
